...
export class PrestantePrestamoController {
  ...
  async find(
    ...
    return this.prestanteRepository.prestamos(id).find(filter);
  }
      ...
      content: {
        'application/json': {
          schema: getModelSchemaRef(Prestamo, {
            ...
            exclude: ['idPrestamo'],
            optional: ['prestanteId']
          }),
        },
      },
    }) prestamo: Omit<Prestamo, 'idPrestamo'>,
  ): Promise<Prestamo> {
    return this.prestanteRepository.prestamos(id).create(prestamo);
  }
  ...
  async patch(
    ...
    return this.prestanteRepository.prestamos(id).patch(prestamo, where);
  }
  ...
  async delete(
    ...
    return this.prestanteRepository.prestamos(id).delete(where);
  }
}
